...
cargo build --release
NATIVE_BIN="$BUILD_DIR/release/client-server"
if [ -f "$NATIVE_BIN" ]; then
    cp "$NATIVE_BIN" "$RELEASE_DIR/client-server-linux-x86_64"
...
if rustup target list | grep -q "x86_64-pc-windows-gnu (installed)"; then
    ...
    WIN_BIN="$BUILD_DIR/x86_64-pc-windows-gnu/release/client-server.exe"
    if [ -f "$WIN_BIN" ]; then
        cp "$WIN_BIN" "$RELEASE_DIR/client-server-windows-x86_64.exe"
