...
/// Run the embedded server. This mirrors the logic from the original `server/main.rs`.
pub async fn run_server() -> std::io::Result<()> {
    ...
    tokio::spawn(async move {
        crate::server::song_watcher::start_watcher(pool_clone).await;
    });
